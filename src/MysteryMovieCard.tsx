import { Card, CardHeader, Divider, Flex, Skeleton, Text } from "@chakra-ui/react";
...
export default function MysteryMovie({ comparison, finishedGame }: MysteryMovieCardProps) {
    ...
    function formatMysteryString(text: string) {
        if (text === "") {
            return <Skeleton height="1rem" width="4rem"/>
        } else {
    ...
    function formatList(list: ListItem[]) {
        let items = [<Skeleton height="1rem" width="4rem"/>, <Skeleton height="1rem" width="4rem"/>, <Skeleton height="1rem" width="4rem"/>]

        for (let i = 0; i < 3; i++) {
            if (list[i] != undefined) {
                items[i] = <Text color="green.400">{list[i].name}</Text>
            }
        }
